...
public class DraggableRow extends LinearLayout implements Checkable {
	...
	public void onFinishInflate() {
		...
		mRatingView    = this.findViewById(R.id.rating);
	...
	public void showRating(boolean state) {
		mRatingView.setVisibility( state ? View.VISIBLE : View.INVISIBLE );
	}
	...
	public void setDuration(long duration) {
		...
		sb.setSpan(new ForegroundColorSpan(Color.GRAY), 0, sb.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		mDurationView.setText(sb);
	}

	public void setRating(double rating, int mediaType) {
		setRating(rating, mediaType, 0);
	}

	public void setRating(double rating, int mediaType, int albumSize) {
		String txt = null;
		if (rating > 0 && mediaType == MediaUtils.TYPE_SONG) {
			txt = String.format(Locale.ROOT, getContext().getString(R.string.song_rating), (int) rating);
		} else if (mediaType == MediaUtils.TYPE_ALBUM || mediaType == MediaUtils.TYPE_PLAYLIST) {
			if (rating > 0) {
				txt = String.format(Locale.ROOT, getContext().getString(R.string.album_size_rating), rating, albumSize);
			} else {
				txt = String.format(Locale.ROOT, getContext().getString(R.string.album_size), albumSize);
			}
		}
		if (txt != null) {
			SpannableStringBuilder sb = new SpannableStringBuilder(txt);
			if (mediaType == MediaUtils.TYPE_ALBUM && rating > 0) {
				sb.setSpan(new RelativeSizeSpan(0.8F), 0, 3, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
				sb.setSpan(new RelativeSizeSpan(0.8F), 4, txt.length() - 1, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
			} else {
				sb.setSpan(new RelativeSizeSpan(0.8F), 0, txt.length() - 1, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
			}
			mRatingView.setText(sb);
			showRating(true);
		} else {
			showRating(false);
		}
